...
def generate_answer_to_scientific_question(question: str, breadth: int, depth: int, diversity: float) -> dict[str, str | list[str]]:
    # Initialize clients
    topic_model = BERTopic.load("MaartenGr/BERTopic_ArXiv")
    embedding_model = SentenceTransformer("sentence-transformers/all-mpnet-base-v2")
    client = MilvusClient("./database.db")
    ...
    # Generate the final related works section text
    question_answer: str = prompting_client.get_completions(
        generate_question_answer_prompt(
            question=question,
            data=relevant_pages
        ),
